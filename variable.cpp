...
std::shared_ptr<Variable> operator/(double const &lhs, std::shared_ptr<Variable> rhs)
{
    std::shared_ptr<DivBackwardConstant> grad_fn = std::make_shared<DivBackwardConstant>();
    grad_fn->self_ = rhs;
    grad_fn->other_ = lhs;
    grad_fn->add_input_nr();
    auto result = std::make_shared<Variable>(lhs / rhs->value_);
...
std::shared_ptr<Variable> operator/(std::shared_ptr<Variable> lhs, double const &rhs)
{
    std::shared_ptr<DivBackwardConstant_R> grad_fn = std::make_shared<DivBackwardConstant_R>();
    grad_fn->self_ = lhs;
    grad_fn->other_ = rhs;
    grad_fn->add_input_nr();
    auto result = std::make_shared<Variable>(lhs->value_ / rhs);
...
std::shared_ptr<Variable> operator^(std::shared_ptr<Variable> lhs, double const &rhs)
{
    std::shared_ptr<PowBackwardConstant> grad_fn = std::make_shared<PowBackwardConstant>();
    grad_fn->self_ = lhs;
    grad_fn->other_ = rhs;
    grad_fn->add_input_nr();
    auto result = std::make_shared<Variable>(std::pow(lhs->value_, rhs));
    result->set_gradient_edge({ grad_fn, 0 });
    grad_fn->add_next_edge(lhs->gradient_edge());
    return result;
}
std::shared_ptr<Variable> operator^(double const &lhs, std::shared_ptr<Variable> rhs)
{
    std::shared_ptr<PowBackwardConstant_R> grad_fn = std::make_shared<PowBackwardConstant_R>();
    grad_fn->self_ = rhs;
    grad_fn->other_ = lhs;
    grad_fn->add_input_nr();
    auto result = std::make_shared<Variable>(std::pow(lhs, rhs->value_));
    result->set_gradient_edge({ grad_fn, 0 });
    grad_fn->add_next_edge(rhs->gradient_edge());
    return result;
}
